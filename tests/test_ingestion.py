...
def test_extract_day_ahead_prices_returns_raw_payload() -> None:
    client = Mock(spec=EntsoeClient)
    client.fetch_day_ahead_prices.return_value = b"<Publication_MarketDocument />"
    ...
    assert payload.content == b"<Publication_MarketDocument />"

    client.fetch_day_ahead_prices.assert_called_once_with(
        bidding_zone="10YFR-RTE------C",
        period_start=period_start,
        period_end=period_end,
    )
